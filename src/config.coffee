...
module.exports = {
  ...
  templates: {
    ...
    institute: [
      ...
      {t: 'text', text: '{{about}}', collapsible: true}
      {t: 'related', title: 'Gruppi di Ricerca', type: 'list', query: "FOR v, e IN ANY @id GRAPH 'graph' FILTER e.type == 'in' AND v.type == 'Gruppo di Ricerca' RETURN v"} # {t: 'related', title: 'Gruppi di Ricerca', type: 'list', query: "MATCH (:Info {id: {current}})-[{type: 'in'}]-(n:Info {type: 'Gruppo di Ricerca'}) RETURN DISTINCT(n) ORDER BY n.label ASC"}
      {t: 'related', title: 'Servizi Generali', type: 'list', query: "FOR v, e IN ANY @id GRAPH 'graph' FILTER e.type == 'in' AND v.type == 'Servizi Generali' RETURN v"} # {t: 'related', title: 'Servizi Generali', type: 'list', query: "MATCH (:Info {id: {current}})-[{type: 'in'}]-(n:Info {type: 'Servizi Generali'}) RETURN DISTINCT(n) ORDER BY n.label ASC"}
      {t: 'related', title: 'Struttura Tecnica di Servizio', type: 'list', query: "FOR v, e IN ANY @id GRAPH 'graph' FILTER e.type == 'in' AND v.type == 'Struttura Tecnica di Servizio' RETURN v"} # {t: 'related', title: 'Strutture Tecniche di Servizio', type: 'list', query: "MATCH (:Info {id: {current}})-[{type: 'in'}]-(n:Info {type: 'Struttura Tecnica di Servizio'}) RETURN DISTINCT(n) ORDER BY n.label ASC"}
    ],
    group: [
      {t: 'image'}
      ...
      {t: 'related', title: 'Persone', aggregator: 'position', query: "FOR v, e IN ANY @id GRAPH 'graph' FILTER e.type == 'in' AND v.template == 'person' SORT v.label RETURN v"} # {t: 'related', title: 'Persone', aggregator: 'position', query: "MATCH (:Info {id: {current}})-[{type: 'in'}]-(n:Info {template: 'person'}) RETURN DISTINCT(n) ORDER BY n.label ASC"}
    ],
    person: [
    ...
    room: [
      ...
      {t: 'related', title: 'Persone', query: "FOR v, e IN ANY @id GRAPH 'graph' FILTER e.type == 'in' SORT v.label RETURN v"} # "MATCH ({id: {current}})-[{type: 'in'}]-(n) RETURN DISTINCT(n) ORDER BY n.label ASC"}
    ]
  }
}
