<template>
  <g v-if="target !== undefined" class="placemark" :transform="get_translate()">
    <g :transform="get_scale()">
      <g transform="scale(10) translate(-10, -34)">
        ...
        <foreignObject width="20" height="20" transform="translate(5.2,1.5)">
          <!--<i class="icon-"></i>-->
        </foreignObject>
      </g>
    </g>
...
export default {
  ...
  methods:
    get_translate: () -> if @target? and @target.position? then "translate(#{@target.position[0]}, #{@target.position[1]})" else ""
