...
module.exports = {
  ...
  query_space: (context, id) ->
    ...
    payload = {query: "MATCH (s1:Space {id: {id}}) WITH COLLECT(s1) AS space OPTIONAL MATCH (s1)-[r1 {type: 'in_list'}]->(n) WITH space,n OPTIONAL MATCH (s)-[r {type: 'in_list'}]->(n) WITH space+COLLECT(s) AS spaces UNWIND spaces AS s RETURN DISTINCT s ORDER BY s.order", params: {id: id}}
    @execute payload, (data) =>
