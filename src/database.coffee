...
module.exports = {
  ...
  query_starting_point: (context, starting_point) ->
    payload = JSON.stringify({query: "MATCH (n:Info) WHERE n.id={starting_point} RETURN n;", params: {starting_point: starting_point}})
    @cypher payload, (data) ->
  ...
  query_nodes: (context, id) ->
    payload = JSON.stringify({query: "MATCH (n:Info)-[]-(a:Annotation)-[]-(s:Space {id: {id}}) RETURN n, a.x, a.y;", params: {id: id}})
    @cypher payload, (data) ->
  ...
  query_directions: (context, from_id, to_id) ->
    ...
    payload = JSON.stringify({query: "OPTIONAL MATCH (start:Info) WHERE start.id={from_id} OPTIONAL MATCH (end:Info) WHERE end.id={to_id} RETURN start, end", params: {from_id: from_id, to_id: to_id}})
    @cypher payload, (data) ->
      result = JSON.parse(data.responseText)
  ...
  query_node: (str, callback) ->
    payload = JSON.stringify({query: "MATCH (n:Info) WHERE lower(n.label) CONTAINS {str} RETURN n LIMIT 5", params: {str: str.toLowerCase()}})
    @cypher payload, callback
